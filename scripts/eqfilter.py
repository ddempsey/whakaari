...
def filtered_accuracy(i):
    ...
    fm = ForecastModel(window=2., overlap=0.75, look_forward=2., data_streams=data_streams,
        root='filtered_e{:d}'.format(i+1), savefile_type='pkl', station='WIZ',
...
def filtered_falsealarms():
    # setup forecast model
    n_jobs = 8
    data_streams = ['rsamF','mfF','hfF','dsarF']
    fm = ForecastModel(window=2., overlap=0.75, look_forward=2., data_streams=data_streams,
        root='filtered_e0', savefile_type='pkl', station='WIZ',
        feature_dir=r'E:\whakaari\features')   
    #fm.data.update()
    ...
    conf = []
    for i in [1,2,3,5]:
        with open(fm.plotdir+'/../filtered_e{:d}/confidence.txt'.format(i)) as fp:
...
def model():
    # test()
    filtered_falsealarms()    
    # return

    # build and run filtered forecast model
    # for i in range(5):
    #     filtered_accuracy(i)    
